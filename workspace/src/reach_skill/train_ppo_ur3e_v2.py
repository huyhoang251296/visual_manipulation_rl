...
ACTION_MODEL_VERSION = "action_v0.2"
...
def main():
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(args.num_steps):
            ...
            clipped_gradient_action = np.clip(
                action,
                action - delta_action_limit,
                action + delta_action_limit
                )

            clipped_action = np.clip(clipped_gradient_action, act_space.low, act_space.high)
